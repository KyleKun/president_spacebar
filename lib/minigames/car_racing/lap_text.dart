...
class LapText extends PositionComponent with HasGameReference<RacingGame> {
  ...
  Future<void> onLoad() async {
    await super.onLoad();
    final textStyle = GoogleFonts.vt323(fontSize: 35, color: car.paint.color);
    final defaultRenderer = TextPaint(style: textStyle);
    final lapCountRenderer = TextPaint(style: textStyle.copyWith(fontSize: 55, fontWeight: FontWeight.bold));
    add(TextComponent(text: 'Lap', position: Vector2(0, -20), anchor: Anchor.center, textRenderer: defaultRenderer));
    final lapCounter = TextComponent(position: Vector2(0, 10), anchor: Anchor.center, textRenderer: lapCountRenderer);
    add(lapCounter);
    void updateLapText() {
      if (lapNotifier.value <= RacingGame.numberOfLaps) {
