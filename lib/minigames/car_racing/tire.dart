...
import 'package:president_spacebar/minigames/car_racing/trail.dart';

class Tire extends BodyComponent<RacingGame> {
  Tire({
    required this.car,
  ...
  // Increased drive force and lateral impulse for better handling
  static const double _backTireMaxDriveForce = 800.0;
  static const double _frontTireMaxDriveForce = 1000.0;
  static const double _backTireMaxLateralImpulse = 12.0;
  static const double _frontTireMaxLateralImpulse = 10.0;

  final Car car;
  final size = Vector2(0.5, 1.25);
  late final RRect _renderRect = RRect.fromLTRBR(-size.x, -size.y, size.x, size.y, const Radius.circular(0.3));

  final Set<LogicalKeyboardKey> pressedKeys;

  late final double _maxDriveForce = isFrontTire ? _frontTireMaxDriveForce : _backTireMaxDriveForce;
  late final double _maxLateralImpulse = isFrontTire ? _frontTireMaxLateralImpulse : _backTireMaxLateralImpulse;
  ...
  @override
  Body createBody() {
    final jointAnchor = Vector2(isLeftTire ? -3.0 : 3.0, isFrontTire ? 3.5 : -4.25);
  ...
  void _updateFriction() {
    ...
    body.applyAngularImpulse(0.1 * _currentTraction * body.getInertia() * -body.angularVelocity);
    ...
    body.applyForce(currentForwardNormal..scale(_currentTraction * dragForceMagnitude));
  }
  ...
  void _updateTurn(double dt) {
    ...
    if (isTurnableTire && isTurning) {
      ...
      final angleToTurn = (desiredAngle - angleNow).clamp(-turnPerTimeStep, turnPerTimeStep);
  ...
  Vector2 get _lateralVelocity {
    ...
    return currentRightNormal..scale(currentRightNormal.dot(body.linearVelocity));
  }

  Vector2 get _forwardVelocity {
    final currentForwardNormal = body.worldVector(_worldUp);
    return currentForwardNormal..scale(currentForwardNormal.dot(body.linearVelocity));
  }
}
