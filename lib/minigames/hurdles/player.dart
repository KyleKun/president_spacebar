...
class Player extends SpriteAnimationGroupComponent<PlayerState> with HasGameReference<HurdlesGame>, CollisionCallbacks {
  ...
  Future<void> onLoad() async {
    // Body hitbox
    add(RectangleHitbox.relative(Vector2(0.7, 0.6), position: Vector2(0, height / 3), parentSize: size));
    // Head hitbox
    add(RectangleHitbox.relative(Vector2(0.45, 0.35), position: Vector2(width / 2, 0), parentSize: size));
    animations = {
      ...
      PlayerState.waiting: _getAnimation(size: Vector2(88.0, 90.0), frames: [Vector2(76.0, 6.0)]),
      PlayerState.jumping: _getAnimation(size: Vector2(88.0, 90.0), frames: [Vector2(1339.0, 6.0)]),
      PlayerState.crashed: _getAnimation(size: Vector2(88.0, 90.0), frames: [Vector2(1782.0, 6.0)]),
    };
  ...
  SpriteAnimation _getAnimation({
    ...
    return SpriteAnimation.spriteList(
      frames.map((vector) => Sprite(game.spriteImage, srcSize: size, srcPosition: vector)).toList(),
      stepTime: stepTime,
    );
  }
}
