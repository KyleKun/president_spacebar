...
import 'package:president_spacebar/minigames/hurdles/background/cloud_manager.dart';
import 'package:president_spacebar/minigames/hurdles/hurdle/hurdle_manager.dart';
import 'package:president_spacebar/minigames/hurdles/hurdles_game.dart';

class Horizon extends PositionComponent with HasGameReference<HurdlesGame> {
  Horizon() : super();
  ...
  late final _softSprite = Sprite(game.spriteImage, srcPosition: Vector2(2.0, 104.0), srcSize: lineSize);
  ...
  List<SpriteComponent> _generateLines() {
    ...
    return List.generate(
      max(number, 0),
      (i) =>
          SpriteComponent(sprite: (i + groundLayers.length).isEven ? _softSprite : _bumpySprite, size: lineSize)
